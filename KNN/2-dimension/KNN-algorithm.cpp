...
void input(std::string path, std::vector<double> &Data1, std::vector<double> &Data2){
...
void KNN(std::vector<double> Data1, std::vector<double> Data2, int k){
	...
	for(int index=0; index<Data1.size(); index++){
		...
		for(int i=0; i<Data1.size(); i++){
			if(i != index){
				temp.insert( sqrt(pow((Data1[index]-Data1[i]),2) + pow((Data2[index]-Data2[i]),2)) );
			}
		}
		int pos = 0;
		for(auto u : temp){
	...
	for(int i=0; i<density.size(); i++){
		...
		out<<Data1[i]<<" "<<Data2[i]<<" "<<density[i] << std::endl;
	}
	out.close();
}
